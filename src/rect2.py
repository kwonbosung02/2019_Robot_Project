...
if __name__ == "__main__":
    ...
    while camera.isOpened():
        ...
        cv2.rectangle(frame, (int(cap_region_x_begin * frame.shape[1]), 50),  (frame.shape[1], int(cap_region_y_end * frame.shape[0])), (255, 0, 0), 2)
        cv2.imshow('original', frame)

        if is_background_captured == 1:
            img = remove_background(frame)
            img = img[50:int(cap_region_y_end * frame.shape[0]), int(cap_region_x_begin * frame.shape[1]):frame.shape[1]]
            ...
            if length > 0:
                ...
                for cnt in contours:
                    try:
                        ...
                        pass
                cv2.setMouseCallback('draw_obj', CallBackFunction)
                res = contours[c_i]
                ...
                cv2.rectangle(draw_Object,(0,0),(int(MAX_x*0.2),MAX_y),(100,0,255),2);
                cv2.rectangle(draw_Object,(int(MAX_x*0.8),0),(MAX_x,MAX_y),(100,0,255),2);
                cv2.rectangle(draw_Object,(int(MAX_x*0.2),int(MAX_y*0.82)),(int(MAX_x*0.5),MAX_y),(255,0,255),2);
                cv2.rectangle(draw_Object,(int(MAX_x*0.5),int(MAX_y*0.82)),(int(MAX_x*0.8),MAX_y),(255,0,255),2);
